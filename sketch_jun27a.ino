

 int LED_PIN_R = 9;   // デジタルピン9番に赤色LEDを割り当てる(変数)
 int LED_PIN_G = 10;  // デジタルピン10番に緑色LEDを割り当てる(変数)
 int LED_PIN_B = 11;  // デジタルピン11番に青色LEDを割り当てる(変数)


//int led=11;
//cc midi messages three bytes
byte statusByte;
byte ControllerNumber;
byte ControllerValue;
//cc midi message channel 1  1011 0000
byte ccmidi = 176;

void setup(){
  //test the leds

    pinMode(LED_PIN_R, OUTPUT);  // それぞれのピンを出力に設定
    pinMode(LED_PIN_G, OUTPUT);
    pinMode(LED_PIN_B, OUTPUT);

     for(int j=255;j>=0;j--){
      analogWrite(LED_PIN_R,j);
      delay(1);
     }     
      analogWrite(LED_PIN_R ,0);

      for(int j=255;j>=0;j--){
      analogWrite(LED_PIN_G,j);
      delay(1);
     }     
      analogWrite(LED_PIN_G ,0);

      for(int j=255;j>=0;j--){
      analogWrite(LED_PIN_B,j);
      delay(1);
     }     
      analogWrite(LED_PIN_B ,0);
    
  //start the serial into the boud rate of 115200
  Serial.begin(115200);
}
void loop(){
  //waite for a midi message
  while(Serial.available() <3);
  //run the function that will control the leds
  readmidi();
  
  }
void readmidi(){
  
  do{
    //sheck if there is any message
    if (Serial.available()){
      //store the data into the variables 
      statusByte = Serial.read();
      ControllerNumber = Serial.read();
      ControllerValue = Serial.read(); 
      //check if the message is a cc midi message
      if (statusByte==ccmidi){

        //controll the leds acording to the data bytes
        
          if(ControllerNumber==22) analogWrite(LED_PIN_R,255-ControllerValue*2);
          if(ControllerNumber==23) analogWrite(LED_PIN_G,255-ControllerValue*2);
          if(ControllerNumber==24) analogWrite(LED_PIN_B,255-ControllerValue*2);
        
      }
    }
  }
  //repeat until there is no message
  while (Serial.available() > 2);
}
